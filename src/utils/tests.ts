...
function encodeOrderFulfilledData({ orderHash, offerer, zone, recipient, offer, consideration }: any) {
    ...
    const data = iface.encodeEventLog(fragment, [orderHash, offerer, zone, recipient, offer, consideration]);
    return data;
}
...
import { NftTokenType, OpenSeaCollectionMetadata, NftContract } from 'alchemy-sdk';
...
export const createBatchContractInfo = (
    ...
    for (let i = 0; i < tokenId.length; i++) {
        ...
        consideration.push({
            ...
            identifier: "0",
            amount: ethers.parseEther(price[i].toString()),
            recipient: to
        });   
    }
